package com.yq.base.common.string;
...
public class StringUtils {
    ...
    public static String format(String needReplace, int res) {
        String str = KitBaseApp.getContext().getResources().getString(res);
        if (str != null && !TextUtils.isEmpty(str)) {
            return MessageFormat.format(str, needReplace);
        } else {
            return null;
        }
    }
...
//        style.setSpan(new BackgroundColorSpan(Color.RED),start,end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);设置字体背景颜色值
    ...
    public static String format(Object[] params, int res) {
        String str = KitBaseApp.getContext().getApplicationContext()
                .getString(res);
        if (str != null && !TextUtils.isEmpty(str)) {
